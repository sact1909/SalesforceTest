...
public sealed class HandleOAuthCallbackService
{
    ...
    public async Task<Result> ExecuteAsync(string code, string state, CancellationToken cancellationToken = default)
    {
        ...
        SalesforceTokenResponse tokens;
        SalesforceUserInfo userInfo;
        try
        {
            tokens = await _oauthService.ExchangeCodeAsync(code, cancellationToken);
            userInfo = await _oauthService.GetUserInfoAsync(tokens.InstanceUrl, tokens.AccessToken, cancellationToken);
        }
