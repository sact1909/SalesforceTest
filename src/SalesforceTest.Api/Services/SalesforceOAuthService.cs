...
internal sealed class SalesforceOAuthService : ISalesforceOAuthService
{
    ...
    public async Task<SalesforceTokenResponse> ExchangeCodeAsync(string code, CancellationToken cancellationToken = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException($"Salesforce token exchange failed ({(int)response.StatusCode}): {errorBody}");
        }
